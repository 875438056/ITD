...
custom_imports = dict(
    imports=['models.backbones.YOLO_SPPF',
             ...
             'models.heads.centroid_adaptive_heatmap_ltrb(iou)_offset_head',
...
litehrnet_extra = dict(
    stem=dict(
        stem_channels=32, # 初始卷积层的输出通道数（第一层卷积后的特征图通道数）
        out_channels=32,  # Stem模块的最终输出通道数（需与后续stage输入匹配）
        expand_ratio=1),  # 分支通道的扩展比率（1表示不扩展，用于轻量化设计）
    num_stages=3,  #  阶段数（通常为 3 或 4）。纵向串联分支的深度, 与num_channels配合使用
    stages_spec=dict(  # 每个阶段的详细配置
        num_modules=(2, 4, 2), # 每个stage包含的模块数量（如stage0有2个LiteHRModule）
        num_branches=(2, 3, 4),  # 横向并行分支，代表有三个不同分辨率的网络分支 stage0:2分支, stage1:3分支, stage2:4分支
        num_blocks=(2, 2, 2),
        module_type=('LITE', 'LITE', 'LITE'), # 'LITE'（条件通道加权）或 'NAIVE'（常规 ShuffleUnit）
        with_fuse=(True, True, True), # 是否启用多分支特征融合
        reduce_ratios=(8, 8, 8),  # 通道压缩比率（如8表示通道数减少到1/8）
        num_channels=(
            (40, 80),            # stage:0  深度为2  featuremap size: stride: /2, /4          channel：[分支1:40, 分支2:80]
            (40, 80, 160),       # stage:1  深度为3  featuremap size: stride: /2, /4, /8      channel：[40, 80, 160]
            (40, 80, 160, 320),  # stage:2  深度为4  featuremap size: stride: /2, /4, /8, 16/ channel：[40, 80, 160, 320]
        )),
...
model = dict(
    ...
    # 检测头：CenterNet 的核心 'AdaptiveLTRBIoUCentroidNetHead'
    bbox_head=dict(
        type='AdaptiveLTRBCentroidNetHead',
        ...
        #     type='KLDivergenceLoss', loss_weight=1.0),
        # loss_center_heatmap=dict(
        #     type='GaussianFocalLoss',
        #     use_focal=True,
        #     alpha=2.0,  # 正样本focal权重
        #     beta=4.0,  # 负样本focal权重
        #     temperature=1.0,
        #     loss_weight=1.5
        # ),
        ...
        loss_ltrb=dict(type='L1Loss', loss_weight=1.0),
...
param_scheduler = [
    ...
    dict(
        ...
        by_epoch=True,
        milestones=[10, 20],  # 在第90和120个epoch降低学习率
        gamma=0.1)
]
