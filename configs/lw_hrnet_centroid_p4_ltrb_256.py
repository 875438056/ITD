...
custom_imports = dict(
    imports=['models.backbones.lightweight_hrnet_p2',
             'models.backbones.lightweight_hrnet',
             ...
             'models.heads.centernet_heatmap_ltrb_offset_head',
...
model = dict(
    ...
    backbone=dict(
        type='lightweight_hrnet',  # <-- 在这里直接使用你的自定义 backbone 名称
    ...
    bbox_head=dict(
        ...
        loss_ltrb=dict(type='L1Loss', loss_weight=0.1),
    ...
    test_cfg=dict(
        ...
        max_per_img=10),  # NMS 后的最终目标数量
    # init_cfg=dict(
    #     type='Pretrained',
    #     checkpoint=r"F:\ITD\tools\work_dirs\lw_hrnet_centroid_p2_ltrb_256\20250627_164409\best_coco_bbox_mAP_epoch_131.pth"
    # ),
)
...
test_pipeline = [
    ...
    dict(
        ...
        test_pad_mode=['size_divisor', 32],  # 这是 CenterNet 官方测试时常用的 padding 模式
    ),
    dict(
...
param_scheduler = [
    ...
    dict(
        ...
        by_epoch=True,
        milestones=[40, 80],  # 在第90和120个epoch降低学习率
        gamma=0.1)
]
